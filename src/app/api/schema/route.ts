 ...
 * schema
 *
 * The Blueprint's Event Schema Builder - a company's own event types
 * (payment.received, invoice.created, ...) with required/optional
 * fields and validation rules, enforced by the Kernel's Schema Engine.
 */
...
export async function GET(req: NextRequest) {
  return forward(req, "schema.list");
}

export async function POST(req: NextRequest) {
  return forward(req, "schema.create");
}
