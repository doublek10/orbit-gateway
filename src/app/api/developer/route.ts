 ...
 * developer
 *
 * Same relay-only pattern as every other domain route: the Gateway
 * checks for a credential and forwards everything else to the Kernel's
 * Workflow Engine, which owns API key generation, hashing, and storage.
 */
...
export async function GET(req: NextRequest) {
  return forward(req, "developer.list");
}

export async function POST(req: NextRequest) {
  return forward(req, "developer.create");
}
